...
public static class TranslationChoice
{
                                             ...
                                             int? chosen, int? installed)
    {
        ...
        // 🔴 **And the one we put here ourselves, which MatchingOnline cannot always see.** A local
        // file that has since diverged — edited in the game, or never published — matches no lineage
        // online, so the test above says nothing about it while the translation is plainly already
        // there. That is how a game carrying a hundred lines of somebody's own work was offered the
        // very translation it had been set up from, with Apply lit and nothing asked for.
        //
        ...
        if (report.LocalTranslation is not null && installed is { } already && already == picked.Id)
            return null;
