...
public static class ModSettingControls
{
    ...
    /// <summary>
    /// The language a game's own text is in: every language, plus <see cref="SourceAuto"/> first.
    ///
    ...
    {
        var box = new SearchPicker { Width = width };
        LanguageMark.Fill(box, Languages.All(), new LanguageChoice("auto", null, SourceAuto));
        return box;
    }

    /// <summary>
    /// The words for a source nobody declared — the mod's own (its Options dropdown,
    /// options.json `SourceLang.first`), so the same setting reads the same in both products.
    /// </summary>
    public const string SourceAuto = "auto (Detect)";
